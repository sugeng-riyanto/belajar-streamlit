...
def first_user_form():
    ...
    if st.button("Generate Link for Second User"):
        if user_name and user_signature_canvas.image_data is not None and user_score is not None and second_user_email:
...
def second_user_form(unique_id):
    ...
    st.write(f"First User's Name: {data['user_name']}")
    st.write(f"First User's Score: {data['user_score']}")
    ...
    if st.button("Submit"):
        if second_user_name and second_user_signature_canvas.image_data is not None and second_user_score is not None:
            ...
            first_user_email = final_data['user_name']
            second_user_email = second_user_name
